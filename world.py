...
class QAWorld(DialogPartnerWorld):
    ...
    def parley(self):
        ...
        if self.qbot.observation.get("episode_done"):
            ...
            self.qbot.reset(
                batch_size=self.episode_batch["task"].size(0), retain_actions=False
            )
            self.abot.reset(
                batch_size=self.episode_batch["task"].size(0), retain_actions=False
            )
            ...
            self.episode_batch["image_embed"] = self.abot.embed_image(
                self.episode_batch["image"]
            )
        ...
        self.qbot.observe(
            {"text": qbot_ques["text"] + self.qbot.listen_offset, "id": self.qbot.id}
        )

        # forget answer if abot is memory-less
        if self.opt["memoryless_abot"]:
            self.abot.reset(
                batch_size=self.episode_batch["task"].size(0), retain_actions=True
            )
    ...
    def save_agents(self, save_path):
        ...
        torch.save(
            {"qbot": qbot_state_dict, "abot": abot_state_dict, "opt": self.opt},
            save_path,
        )
