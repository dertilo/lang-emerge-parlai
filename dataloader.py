...
class ShapesQADataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        attr_vals = reduce(
            lambda x, y: x + y, [self.properties[attr] for attr in self.attributes]
        )
        ...
        inv_vocab_attr_val = {
            value: index for index, value in self.vocab_attr_val.items()
        }

        for dtype in ["train", "val"]:
            data_t = torch.LongTensor(len(self.data[dtype]), len(self.properties))
            for index, attr_set in enumerate(self.data[dtype]):
                data_t[index] = torch.LongTensor(
                    [inv_vocab_attr_val[attr] for attr in attr_set]
                )
    ...
    def random_batch(self, dtype, current_pred=None):
        ...
        indices = [
            random.randint(0, len(self.data[dtype]) - 1)
        ...
        if current_pred is not None:
            ...
            neg_indices = current_pred.view(-1, len(self.task_defn)).sum(1) < len(
                self.task_defn
            )
            neg_indices = (
        ...
        tasks = torch.Tensor(
            [
                random.randint(0, len(self.task_defn) - 1)
                for _ in range(self.opt["batch_size"])
            ]
        ).long()
        # now sample predictions based on task
    ...
    def talk_to_script(self, talk, preds, batch):
        ...
        if self.opt["q_out_vocab"] < 4:
            ...
            q_vocab = [
                "Q%d" % i for i in range(self.opt["q_out_vocab"])
            ]  # Q1, Q2, Q3, Q4...
            a_vocab = [
                "A%d" % i for i in range(self.opt["a_out_vocab"])
            ]  # A1, A2, A3, A4...
